...
// The following blocks should be separate to avoid repeating 'describe' for the same suite
describe("Issue create additional tests", () => {
  beforeEach(() => {
    cy.visit("/");
    cy.url()
      .should("eq", `${Cypress.env("baseUrl")}project/board`)
  ...
  it.only("Test Case 2: Random Data Plugin Issue Creation", () => {
    ...
    cy.get('[data-testid="modal:issue-create"]').within(() => {
      ...
      // Select assignee as "Pickle Rick"
      cy.get('[data-testid="form-field:userIds"]').click();
      cy.get('[data-testid="select-option:Pickle Rick"]').click();
